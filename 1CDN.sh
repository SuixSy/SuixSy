...
# 将 ql extra 添加到定时任务
add_ql_extra() {
    if [ "$(grep -c "ql\ extra" /qinglong/config/crontab.list)" != 0 ]; then
        echo "您的任务列表中已存在 task:ql extra"
    else
        echo "开始添加 task:ql extra"
        # 获取token
        token=$(cat /qinglong/config/auth.json | jq --raw-output .token)
...
add_task_code() {
    if [ "$(grep -c "code.sh" /qinglong/config/crontab.list)" != 0 ]; then
        echo "您的任务列表中已存在 task:task code.sh"
    else
        echo "开始添加 task:task code.sh"
        # 获取token
        token=$(cat /qinglong/config/auth.json | jq --raw-output .token)
        curl -s -H 'Accept: application/json' -H "Authorization: Bearer $token" -H 'Content-Type: application/json;charset=UTF-8' -H 'Accept-Language: zh-CN,zh;q=0.9' --data-binary '{"name":"格式化更新助力码","command":"task /qinglong/config/code.sh","schedule":"*/10 * * * *"}' --compressed 'http://127.0.0.1:5700/api/crons?t=1626247939659'
    fi
}
if [ "${all}" = 1 ]; then
...
# 添加定时任务 ql bot
add_ql_bot() {
    if [ "$(grep -c "ql\ bot" /qinglong/config/crontab.list)" != 0 ]; then
...
# 添加定时任务 自动更新模板
add_curl_sample() {
    if [ "$(grep -c "config.sample.sh" /qinglong/config/crontab.list)" != 0 ]; then
        echo "您的任务列表中已存在 task:自动更新模板"
    else
        echo "开始添加 task:curl config.sample.sh"
        # 获取token
        token=$(cat /qinglong/config/auth.json | jq --raw-output .token)
        curl -s -H 'Accept: application/json' -H "Authorization: Bearer $token" -H 'Content-Type: application/json;charset=UTF-8' -H 'Accept-Language: zh-CN,zh;q=0.9' --data-binary '{"name":"自动更新模板","command":"curl -L https://raw.githubusercontents.com/shufflewzc/VIP/main/Conf/Qinglong/config.sample.sh -o /qinglong/sample/config.sample.sh && cp -rf /qinglong/sample/config.sample.sh /qinglong/config","schedule":"45 6,18 * * *"}' --compressed 'http://127.0.0.1:5700/api/crons?t=1627380635389'
    fi
}
run_curl_sample() {
    curl -sL $valid_url -o /qinglong/sample/config.sample.sh && cp -rf /qinglong/sample/config.sample.sh /qinglong/config
}
if [ "${all}" = 1 ]; then
